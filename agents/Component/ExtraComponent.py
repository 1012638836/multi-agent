...
class CategoryRequirementsComponent(ToolComponent):
    def __init__(self, information_path):
        ...
        self.target_embbeding = get_embedding(self.leaf_name)
    ...
    def func(self, agent):
        ...
        print('arguments: {}'.format(response["function_call"].arguments))
        ...
        topk_result = matching_category(category, self.leaf_name, None, self.target_embbeding, top_k=3)
        top1_score = topk_result[1][0]
        request_items, top_category = search_with_api(requirements, category)

        MIN_CATEGORY_SIM = eval(os.environ["MIN_CATEGORY_SIM"]) if "MIN_CATEGORY_SIM" in os.environ else 0.7
