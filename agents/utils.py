...
def get_embedding(sentence):
    ...
    # embed_model_name = ('F:/Pre-trained-Models/all-MiniLM-L6-v2')
    ...
    if not is_load:
        ...
        if embed_model_name in ["text-embedding-ada-002"]:
            ...
            embedding_model = SentenceTransformer(embed_model_name, device="cpu", cache_folder="F:/Pre-trained-Models")

    if embed_model_name in ["text-embedding-ada-002"]:
        # openai.api_key = os.environ["API_KEY"]
        # if "PROXY" in os.environ:
        #     assert ("http:" in os.environ["PROXY"] or "socks" in os.environ["PROXY"]), \
        #         "PROXY error,PROXY must be http or socks"
        #     openai.proxy = os.environ["PROXY"]
        # if "API_BASE" in os.environ:
        #     openai.api_base = os.environ["API_BASE"]
        # print(sentence)
        embed = client.embeddings.create(model=embed_model_name, input=sentence)
    ...
    # embed = [1, embed_dim]
    if len(embed.shape) == 1: embed = embed.unsqueeze(0)
    # print(embed.shape)
    return embed
...
def save_logs(log_path, messages, response):
    ...
    log_file = os.path.join(log_path, datetime.datetime.now().strftime("%Y-%m-%d-%H-%M-%S") + ".json")
...
def matching_category(inputtext, forest_name, requirements=None, cat_embedder=None, top_k=3):
    ...
    print('total_scores: {}'.format(total_scores.shape))

    top_k = top_k if total_scores.shape[0] > top_k else total_scores.shape[0]
...
def get_relevant_history(query, history, embeddings):
    ...
    # print(query, history, embeddings)
